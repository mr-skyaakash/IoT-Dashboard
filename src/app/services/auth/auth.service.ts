...
export class AuthService {
    ...
    login(loginUser: LoginUser) {
        ...
        console.log('login');
            this.http.post('http://172.16.73.41:5000/auth/login', JSON.stringify(user), { headers: this.generateHeader(), observe: 'response'} ).subscribe(res => {
                if ( res.status === 200 ) {
    ...
    signup(signupUser: SignupUser) {
        ...
        this.http.post('http://172.16.73.41:5000/auth/register', user, { headers: this.generateHeader() , observe: 'response'} ).subscribe(resp => {
            if ( resp.status === 201 ) {
                console.log(resp);
                this.router.navigate(['/']);
                    this.status.next(true);
    ...
    logout() {
        ...
        this.http.post('http://172.16.73.41:5000/auth/logout',  { headers: this.generateHeader() , observe: 'response'} ).subscribe(resp => {
            console.log(resp);
        }, err => {
            console.log(err);
        });
